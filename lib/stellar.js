...
export const CONTRACT_ID = "CCKBIHX4IDKT77IVKTQVBA6IYICO2W6PVBCU43QUXZUA4PAH6YC2O2XJ";
export const DEMO_ADDR = "GBKM52FA6XTDQL775MG5KWLAGU3MPHKGU2VRXWZR2FE773YBBULBFUZV";
export const NETWORK_NAME = "TESTNET";
export const STROOPS_PER_XLM = 10_000_000n;
// Wrapped native XLM Stellar Asset Contract on testnet.
export const NATIVE_XLM_SAC = "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC";
// Points-token contract address. Leave as empty string until you deploy the
// `points-token` crate, then drop the new C... id in here.
export const POINTS_TOKEN_ID = "CD5KW3UM3M7SW3UIVP4FYA4GMHXP7M7W2QTCFUR4HKDNZ6ZVH2FUPG5M";
...
const invokeWrite = async (method, args = [], { contractId = CONTRACT_ID } = {}) => {
    requireConfig();
    if (!contractId) throw new Error("Contract ID not configured for this call");
...
const invokeRead = async (method, args = [], { contractId = CONTRACT_ID } = {}) => {
    requireConfig();
    if (!contractId) throw new Error("Contract ID not configured for this call");
...
export const createSurvey = async (payload) => {
    ...
    return invokeWrite("create_survey", [
        ...
        toAddress(payload.rewardToken || NATIVE_XLM_SAC),
    ]);
};
...
export const withdrawUnusedFunds = ({ id, creator }) =>
    invokeWrite("withdraw_unused_funds", [toSymbol(id), toAddress(creator)]);

// --- Response whitelist (gates who may submit) ---

export const enableWhitelist = ({ id, caller }) =>
    invokeWrite("enable_whitelist", [toSymbol(id), toAddress(caller)]);

export const addToWhitelist = ({ id, caller, addresses }) => {
    ...
    return invokeWrite("add_to_whitelist", [toSymbol(id), toAddress(caller), list]);
};

// --- Co-admin management (creator only) ---

export const addCoAdmin = ({ id, creator, address }) =>
    invokeWrite("add_co_admin", [toSymbol(id), toAddress(creator), toAddress(address)]);

export const removeCoAdmin = ({ id, creator, address }) =>
    invokeWrite("remove_co_admin", [toSymbol(id), toAddress(creator), toAddress(address)]);

export const getCoAdmins = (id) => invokeRead("get_co_admins", [toSymbol(id)]);

export const isCoAdmin = (id, address) =>
    invokeRead("is_co_admin", [toSymbol(id), toAddress(address)]);

// --- Visibility & viewer list (admin or co-admin) ---

const toBool = (value) => xdr.ScVal.scvBool(Boolean(value));

export const setVisibility = ({ id, caller, isPrivate }) =>
    invokeWrite("set_visibility", [toSymbol(id), toAddress(caller), toBool(isPrivate)]);

export const addAllowedViewers = ({ id, caller, addresses }) => {
    if (!addresses?.length) throw new Error("At least one address is required");
    const list = xdr.ScVal.scvVec(addresses.map((addr) => toAddress(addr)));
    return invokeWrite("add_allowed_viewers", [toSymbol(id), toAddress(caller), list]);
};

export const removeAllowedViewer = ({ id, caller, address }) =>
    invokeWrite("remove_allowed_viewer", [toSymbol(id), toAddress(caller), toAddress(address)]);

export const getAllowedViewers = (id) => invokeRead("get_allowed_viewers", [toSymbol(id)]);

export const isPrivateSurvey = (id) => invokeRead("is_private", [toSymbol(id)]);

export const canView = (id, address) =>
    invokeRead("can_view", [toSymbol(id), toAddress(address)]);

// --- Survey contract admin & points-token configuration ---

export const initSurveyAdmin = ({ admin }) =>
    invokeWrite("init_admin", [toAddress(admin)]);

export const setSurveyPointsConfig = ({ admin, token, creatorPoints, respondentPoints }) =>
    invokeWrite("set_points_config", [
        toAddress(admin),
        toAddress(token),
        toI128(creatorPoints),
        toI128(respondentPoints),
    ]);

export const getSurveyPointsConfig = () => invokeRead("get_points_config", []);

export const getSurveyContractAdmin = () => invokeRead("get_contract_admin", []);

// --- Points-token contract bindings ---

export const initPointsToken = ({ admin, name, symbol, decimals = 0 }) => {
    if (!POINTS_TOKEN_ID) throw new Error("POINTS_TOKEN_ID is not configured");
    return invokeWrite(
        "initialize",
        [
            toAddress(admin),
        ...
        { contractId: POINTS_TOKEN_ID },
    );
};

export const setPointsTokenMinter = ({ admin, minter }) => {
    if (!POINTS_TOKEN_ID) throw new Error("POINTS_TOKEN_ID is not configured");
    return invokeWrite(
        "set_minter",
        [toAddress(admin), toAddress(minter)],
        { contractId: POINTS_TOKEN_ID },
    );
};

export const transferPoints = ({ from, to, amount }) => {
    if (!POINTS_TOKEN_ID) throw new Error("POINTS_TOKEN_ID is not configured");
    return invokeWrite(
        "transfer",
        [toAddress(from), toAddress(to), toI128(amount)],
        { contractId: POINTS_TOKEN_ID },
    );
};

export const getPointsBalance = (address) => {
    if (!POINTS_TOKEN_ID) return Promise.resolve(0n);
    return invokeRead("balance", [toAddress(address)], { contractId: POINTS_TOKEN_ID });
};

export const getPointsMetadata = async () => {
    if (!POINTS_TOKEN_ID) return null;
    const [name, symbol, decimals, totalSupply] = await Promise.all([
        invokeRead("name", [], { contractId: POINTS_TOKEN_ID }).catch(() => ""),
        invokeRead("symbol", [], { contractId: POINTS_TOKEN_ID }).catch(() => ""),
        invokeRead("decimals", [], { contractId: POINTS_TOKEN_ID }).catch(() => 0),
        invokeRead("total_supply", [], { contractId: POINTS_TOKEN_ID }).catch(() => 0n),
    ]);
    return { name, symbol, decimals: Number(decimals || 0), totalSupply };
};
